...
const Helper = {
  ...
  // Return splitted text in chunks of sentences
  splitTextBySentence(text, minChunkSize = 300) {
    let chunks = [];
    let cursor = 0;
    let maxLength = text.length;

    while (cursor < maxLength) {
      let chunk = text.substring(cursor, cursor + minChunkSize);
      cursor += minChunkSize;

      let rest = text.substring(cursor);
      let nextDotPosition = rest.indexOf('.') + 1;
      let suffix = rest.substring(0, nextDotPosition);
      cursor += nextDotPosition;
